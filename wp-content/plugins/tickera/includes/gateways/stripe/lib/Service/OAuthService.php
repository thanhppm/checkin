...
class OAuthService extends \TCStripe\Service\AbstractService
{
    /**
     ...
     *
     * @return \TCStripe\StripeObject the object returned by Stripe's Connect API
     */
    ...
    public function authorizeUrl($params = null, $opts = null)
    {
        ...
        $query = \TCStripe\Util\Util::encodeParameters($params);
     ...
     * @param null|array $opts
     *
     * @throws \TCStripe\Exception\OAuth\OAuthErrorException if the request fails
     *
    ...
    private function _getClientId($params = null)
    {
        ...
        if (null === $clientId) {
            $msg = 'No client_id provided. (HINT: set your client_id using '
              . '`new \TCStripe\StripeClient([clientId => <CLIENT-ID>
     ...
     *
     * @return \TCStripe\Util\RequestOptions
     */
    private function _parseOpts($opts)
    {
        if (\is_array($opts)) {
            if (\array_key_exists('connect_base', $opts)) {
                ...
                // \TCStripe\Service\OAuthService from \TCStripe\OAuth, where `connect_base`
                // was the name of the parameter that behaves as `api_base` does here.
                throw new \TCStripe\Exception\InvalidArgumentException('Use `api_base`, not `connect_base`');
            }
        }

        return \TCStripe\Util\RequestOptions::parse($opts);
    }
