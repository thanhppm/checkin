...
class BaseStripeClient implements StripeClientInterface
{
    ...
    /** @var \TCStripe\Util\RequestOptions */
    private $defaultOpts;
    ...
    public function __construct($config = [])
    {
        if (\is_string($config)) {
            ...
            throw new \TCStripe\Exception\InvalidArgumentException('$config must be a string or an array');
        }
        ...
        $this->defaultOpts = \TCStripe\Util\RequestOptions::parse([
     ...
     * @param array|\TCStripe\Util\RequestOptions $opts the special modifiers of the request
     *
     * @return \TCStripe\StripeObject the object returned by Stripe's API
     */
    public function request($method, $path, $params, $opts)
    {
        $opts = $this->defaultOpts->merge($opts, true);
        $baseUrl = $opts->apiBase ?: $this->getApiBase();
        $requestor = new \TCStripe\ApiRequestor($this->apiKeyForRequest($opts), $baseUrl);
    ...
    public function requestCollection($method, $path, $params, $opts)
    {
        ...
        if (!($obj instanceof \TCStripe\Collection)) {
            ...
            throw new \TCStripe\Exception\UnexpectedValueException($msg);
        }
    ...
    /**
     * @param \TCStripe\Util\RequestOptions $opts
     *
     * @throws \TCStripe\Exception\AuthenticationException
     *
     * @return string
     */
    private function apiKeyForRequest($opts)
    {
        ...
        if (null === $apiKey) {
            ...
            throw new \TCStripe\Exception\AuthenticationException($msg);
        }
     ...
     *
     * @throws \TCStripe\Exception\InvalidArgumentException
     */
    private function validateConfig($config)
    {
        // api_key
        if (null !== $config['api_key'] && !\is_string($config['api_key'])) {
            throw new \TCStripe\Exception\InvalidArgumentException('api_key must be null or a string');
        }

        if (null !== $config['api_key'] && ('' === $config['api_key'])) {
            $msg = 'api_key cannot be the empty string';

            throw new \TCStripe\Exception\InvalidArgumentException($msg);
        }
        ...
        if (null !== $config['client_id'] && !\is_string($config['client_id'])) {
            throw new \TCStripe\Exception\InvalidArgumentException('client_id must be null or a string');
        }

        // stripe_account
        if (null !== $config['stripe_account'] && !\is_string($config['stripe_account'])) {
            throw new \TCStripe\Exception\InvalidArgumentException('stripe_account must be null or a string');
        }

        // stripe_version
        if (null !== $config['stripe_version'] && !\is_string($config['stripe_version'])) {
            throw new \TCStripe\Exception\InvalidArgumentException('stripe_version must be null or a string');
        }

        // api_base
        if (!\is_string($config['api_base'])) {
            throw new \TCStripe\Exception\InvalidArgumentException('api_base must be a string');
        }

        // connect_base
        if (!\is_string($config['connect_base'])) {
            throw new \TCStripe\Exception\InvalidArgumentException('connect_base must be a string');
        }

        // files_base
        if (!\is_string($config['files_base'])) {
            throw new \TCStripe\Exception\InvalidArgumentException('files_base must be a string');
        }

        // check absence of extra keys
        $extraConfigKeys = \array_diff(\array_keys($config), \array_keys($this->getDefaultConfig()));
        if (!empty($extraConfigKeys)) {
            throw new \TCStripe\Exception\InvalidArgumentException('Found unknown key(s) in configuration array: ' . \implode(',', $extraConfigKeys));
        }
    }
}
